...
void alloc_heap(Heap * h, size_t heap_size) 
{
    ...
    if (mem == NULL) {
        printf( "RTS ERROR: Unable to allocate heap. Requested %d bytes.\n",
                (int)heap_size);
        exit(EXIT_FAILURE);
    }
...
void heap_check_underflow(Heap * heap) {
    if (!(heap->heap <= heap->next)) {
       printf("RTS ERROR: HEAP UNDERFLOW <bot %p> <cur %p>\n", 
               heap->heap, heap->next);
        exit(EXIT_FAILURE);
    }
}
...
void heap_check_overflow(Heap * heap) {
    if (!(heap->next <= heap->end)) {
       printf("RTS ERROR: HEAP OVERFLOW <cur %p> <end %p>\n", 
               heap->next, heap->end);
        exit(EXIT_FAILURE);
    }
}
...
void heap_check_pointers(Heap * heap) {
    ...
    for(scan = heap->heap; scan < heap->next; scan += item_size) {
       ...
       case FWD:
           ...
           printf("RTS ERROR: FWD in working heap.\n");
           exit(EXIT_FAILURE);
           break;
       default:
           break;
